...
LUI.createMenu.T7Hud_zm_castle = function ( controller )
	...
	ZmNotifBGBContainerFactory:subscribeToGlobalModel( controller, "PerController", "scriptNotify", function ( modelRef )
		...
		if IsParamModelEqualToString( modelRef, "zombie_bgb_token_notification" ) then
			AddZombieBGBTokenNotification( self, f21_local0, controller, modelRef )
		elseif IsParamModelEqualToString( modelRef, "zombie_bgb_notification" ) then
			AddZombieBGBNotification( self, f21_local0, modelRef )
		elseif IsParamModelEqualToString( modelRef, "zombie_notification" ) then
			AddZombieNotification( self, f21_local0, modelRef )
		end
	end )
	...
	MPScore:subscribeToGlobalModel( controller, "PerController", "scriptNotify", function ( modelRef )
		...
		if IsParamModelEqualToString( modelRef, "score_event" ) and PropertyIsTrue( self, "menuLoaded" ) then
			...
			SetMPScoreText( f3_local1, f36_local0, controller, modelRef )
		end
	end )
	...
	RocketShield:mergeStateConditions( {
		{
			...
			condition = function ( menu, element, event )
				local f37_local0 = Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_SCOREBOARD_OPEN )
				if f37_local0 then
					f37_local0 = AlwaysFalse()
				end
				return f37_local0
			end
		}
	...
	RagnarokBlueprintWidgetCastle:mergeStateConditions( {
		{
			stateName = "Scoreboard",
			condition = function ( menu, element, event )
				local f40_local0 = Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_SCOREBOARD_OPEN )
				if f40_local0 then
	...
	RagnarokBlueprintWidgetCastle:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "zmInventory.widget_gravityspike_parts" ), function ( modelRef )
	...
	RagnarokBlueprintWidgetCastle:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "UIVisibilityBit." .. Enum.UIVisibilityBit.BIT_SCOREBOARD_OPEN ), function ( modelRef )
		f3_local1:updateElementState( RagnarokBlueprintWidgetCastle, {
			name = "model_validation",
			menu = f3_local1,
			modelValue = Engine.GetModelValue( modelRef ),
	...
	FuseBoxWidget:mergeStateConditions( {
		{
			stateName = "Scoreboard",
			condition = function ( menu, element, event )
				local f43_local0 = Engine.IsVisibilityBitSet( controller, Enum.UIVisibilityBit.BIT_SCOREBOARD_OPEN )
				if f43_local0 then
					f43_local0 = AlwaysFalse()
				end
				return f43_local0
			end
		}
	} )
	FuseBoxWidget:subscribeToModel( Engine.GetModel( Engine.GetModelForController( controller ), "zmInventory.widget_sprayer" ), function ( modelRef )
